...
def validate_route(backend):
    ...
    # not have those elements, the back end must be marked as "public".
    if not ("auth" in backend or "source_ip" in backend):
        is_public: typing.Optional[bool] = backend.get("public")

        if not is_public:
            raise InvalidBackendDefinitionError(
                f'[backend_name: {name}] The back end does not have either an "auth" or "source_ip" definitions, nor it has been marked as "public". Either add an access restriction mechanism, or add a "public: true" line in the back end\'s definition'
            )
