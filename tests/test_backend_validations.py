...
class TestBackendValidations(unittest.TestCase):
    ...
    def test_restricted_public_routes(self):
        """Tests that a back end which is not restricted by either "auth" or "source_ip" and that does not have the "public" key raises a validation error."""

        backend = dict(
            name="public-unrestricted",
            route="/public/unrestricted",
        ...
        self.assertEqual(
            f'[backend_name: {backend["name"]}] The back end does not have either an "auth" or "source_ip" definitions, nor it has been marked as "public". Either add an access restriction mechanism, or add a "public: true" line in the back end\'s definition',
            str(cm.exception),
        )
