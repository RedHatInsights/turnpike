...
class TestOIDCAuthDebugGating(unittest.TestCase):
    ...
    def test_unauthorized_client_id_suppressed(self):
        ...
        messages = " ".join(cm.output)
        self.assertNotIn("Token client ID not authorized", messages)
        self.assertNotIn(token.claims["clientId"], messages)
    ...
    def test_decode_failure_debug_suppressed(self):
        """JWT decode failure debug details must not appear in logs when AUTH_DEBUG is off."""
        backend = self._find_jwt_backend()["auth"]
        context = mock.Mock
        get_jwks_keyset = mock.Mock
        ...
        messages = " ".join(cm.output)
        self.assertNotIn("JWT decode failure details", messages)
        self.assertIn("Unable to decode the incoming JWT", messages)
        self.assertEqual(http.HTTPStatus.UNAUTHORIZED, context.status_code)
