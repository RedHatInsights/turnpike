...
from turnpike import create_app, Backend
...
class TestVPNPlugin(TestCase):
    default_backend = Backend(
        {
            ...
            "route": "http://test/vpn/plugin",
            "private": True,
        }
    )

    def setUpVPNPlugin(self, environment: str, backend: Optional[Backend] = None) -> VPNPlugin:
        if not backend:
    ...
    def test_private_backend_invalid_edge_host_header(self):
        ...
        vpn_plugin = self.setUpVPNPlugin("stage")
        context = PolicyContext(self.default_backend)
