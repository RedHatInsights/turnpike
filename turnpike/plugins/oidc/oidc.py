...
class OIDCAuthPlugin(TurnpikeAuthPlugin):
    ...
    def __init__(self, app):
        ...
        it_host = app.config["IT_SERVICE_HOST"]
        it_port = app.config["IT_SERVICE_PORT"]
        it_scheme = app.config["IT_SERVICE_PROTOCOL_SCHEME"]
    ...
    def _get_jwks_keyset(self) -> KeySet:
        ...
        if not jwks_certificates:
            ...
            # Store the retrieved certificates for 24 hours, which is the recommended caching setting for the
            # certificates as suggested by IT.
    ...
    def process(self, context, backend_auth: Dict[str, Dict[str, list[Dict[str, Union[str, list[str]]]]]]):
        ...
        if bearer_token.startswith("Bearer"):
            bearer_token: str = re.sub("Bearer\\s+", "", bearer_token)
        ...
        token_client_id: Optional[str] = token.claims.get("client_id")
        if not token_client_id:
            self.app.logger.debug(f'The received token does not contain the "client_id" claim')
        ...
        service_accounts: list[Dict[str, Union[str, list[str]]]] = backend_auth[self.principal_type]["serviceAccounts"]
        ...
        expected_scopes: typing.Optional[list[str], None] = target_sa.get("scopes")
        if expected_scopes:
            token_scope: typing.Optional[str, None] = token.claims.get("scope")
            if not token_scope:
                self.app.logger.debug(
                    f'The request is denied because the incoming token with client id "{token_client_id}" does not contain any scopes'
                )

                context.status_code = http.HTTPStatus.UNAUTHORIZED
                return context

            token_scopes: list[str] = token_scope.split(" ")
        ...
        context.auth = dict(auth_data={"client_id": token_client_id}, auth_plugin=self)
        return context
